...
//    $remove_link = preg_replace('/([?&])ws=[^&]+(&|$)/','$1',$remove_link);
    ...
    foreach($custom_terms as $custom_term) {
        $childs = get_term_children( $custom_term->term_id ,'w_dalil_category' );
        ...
        if($childs){
            ...
            foreach($childs as $child ){
                $child = get_term_by( 'id' , $child ,'w_dalil_category' );
                echo '<li><a class="listcat_item_child" href="'.$remove_link.$getexistes.'wcat='.$child->name.'" >'.$child->name.'</a></li>';
            }
            echo '</ul>';
        }
    }
